...
function basic () {
	proceed=false
	echo "WebSocket BF v1.1 ( github.com/ivan-sincek/websocket-bf )"
	echo ""
	echo "--- Single request ---"
...
if [[ $# == 0 ]]; then
	...
	if [[ ${args[domain]} == "" || ${args[payload]} == "" || $(check $# args) -eq false ]]; then
...
function get_sid () {
	...
	curl -s -H "Connection: close" -H "Accept-Encoding: gzip, deflate" -H "Authorization: Bearer ${2:-null}" "${1}/socket.io/?EIO=3&transport=polling" | gunzip -c | grep -P -o "(\{(?:[^\{\}]+|(?-1))+\})" | jq -r ".sid"
}
...
if [[ $proceed == true ]]; then
	...
	if [[ ${args[wordlist]} != "" ]]; then
		...
		for entry in $(cat ${args[wordlist]}); do
			...
			results=$(fetch_results ${args[domain]} $sid ${args[token]})
			echo $results
		done
	else
		sid=$(get_sid ${args[domain]} ${args[token]})
